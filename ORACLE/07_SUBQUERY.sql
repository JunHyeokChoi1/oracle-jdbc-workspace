...
FROM EMPLOYEE WHERE SALARY > (SELECT ROUND(AVG(NVL(SALARY, 0))) FROM EMPLOYEE);

/*
서브쿼리의 종류
서브쿼리를 수행한 결과값이 몇 행 열이냐에 따라 분류됨
서브쿼리의 종류에 따라 서브쿼릐 앞에 붙는 연산자가 달라짐
*/
/*
1. 단일행 서브쿼리(SINGLE ROW SUBQUERY)
서브쿼리의 조회 결과값이 개수가 오로지 1개일 때 (한행 한열)
일반 비교연산자 사용 가능 : =, !=, ^=, >, <, >=, ...
*/

-- 노옹철 사원의 급여보다 더 많이 받는 사원의 사번, 사원명, 부서명, 직급코드, 급여조회
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_CODE FROM EMPLOYEE E
...
SELECT DEPT_CODE, SUM(SALARY) FROM EMPLOYEE GROUP BY DEPT_CODE HAVING SUM(SALARY) = (SELECT MAX(SUM(SALARY)) FROM EMPLOYEE GROUP BY DEPT_CODE);

--전지연 사원에 속해있는 부서원들의 사번, 사원명, 전화번호, 직급명, 부서명, 입사일 조회(단, 전지연 사원은 제외)
SELECT EMP_ID, EMP_NAME, PHONE, JOB_NAME, DEPT_TITLE, HIRE_DATE FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '전지연' 
AND EMP_NAME != '전지연');

SELECT EMP_ID, EMP_NAME, PHONE, JOB_NAME, DEPT_TITLE, HIRE_DATE FROM EMPLOYEE E, JOB J, DEPARTMENT D
...
-- 8000000, 3900000, 3660000, 2550000, 2890000, 3760000, 2490000
-- 위의 급여를 받는 사원들 조회
...
--에 대해 사원 이름 부서 코드 구분(사수/사원) 조회
--사수에 해당하는 사번을 조회
SELECT DISTINCT MANAGER_ID
FROM EMPLOYEE 
WHERE MANAGER_ID IS NOT NULL;

--사번이 위와 같은 직원들의 사번, 이름, 부서코드, 구부(사수) 조회
SELECT EMP_NO, EMP_NAME, DEPT_CODE, '사수' AS "구분"
FROM EMPLOYEE
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID 
                    FROM EMPLOYEE 
                    WHERE MANAGER_ID IS NOT NULL);
--일반 사원에 해당하는 직원들의 사번, 이름 부서코드, 구부(사원 조회)
SELECT EMP_NO, EMP_NAME, DEPT_CODE, '사원' AS "구분"
FROM EMPLOYEE
...
비교대상 < ANY(서브쿼리) : 여러개의 결과값 중에서 "한개라도" 작을경우(여러개의 결과값 중에서 가장 큰 값 보다 작을 경우)
*/
...
--> 오라클 구문
SELECT SALARYY FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE
AND J.JOB_NAME = '과장';
...
-- 직급이 대리인 직원들 중에서 위의 값의 몰록 중에 하나라도 큰 경우
--오라클 구문
...
/*
다중행 다중열 서브쿼리
서브쿼리의 조회 결과값이 여러 행, 여려 열일 경우(여러행 여러열)
*/
...
--부서별 평균 급여가 높은 3개의 부서의 부서 코드, 평급 급여 조회
--1)인라인 뷰를 사용하는 방법
