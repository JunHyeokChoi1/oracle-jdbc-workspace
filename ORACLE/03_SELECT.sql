...
--EPLOYEE 테이블에서 직원명, 보너스가 포함 된 연봉(급여+(보너스*급여*12) 조회
SELECT EMP_NAME,(SALARY + (SALARY*BONUS)) * 12 FROM EMPLOYEE

--EMPLOYEE 사원명 입사일, 근무일수(오늘날짜-입사일) 조회
--오늘날짜 : SYSDATE
--CEIL(NUMBER) : 매개값으로 전달되는 수를 올림하는 함수
SELECT EMP_NAME, HIRE_DATE, CEIL(SYSDATE-HIRE_DATE) FROM EMPLOYEE;

/*
컬럼 별칭
[표현법]
컬럼 AS 별칭/ 컬럼 AS "별칭" / 컬럼 별칭 / 컬럼 "별칭"
산술연살을 하게 되면 컬럼명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
별칭을 부열할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰따옴표("")로 감싸줘야 한다.
*/

--EPMLOYEE 테이블에서 직원명, 연봉, 보너스가 포함된 연봉((급여+(보너스*급여))*12 조회
SELECT EMP_NAME AS 직원명, SALARY * 12 AS "연봉", (SALARY + (BONUS * SALARY)) * 12 "보너스" FROM EMPLOYEE;

/*
리터럴
select절에 리터럴을 사용하면 테이블에 존재하는 테이터처럼 조회가 가능
즉 리터럴은 RESULT SET의 모든 행에 반복적으로 출력된다.

*/
--employee 테이블에서 사번, 직원명, 급여, (원)조회 
SELECT EMP_ID AS 사번, EMP_NAME AS "직원명", SALARY "급여", '원'AS"단위(원)" FROM EMPLOYEE;
...
-- xxx(사원명)의 월급은 xxx(급여)원 입니다. "급여정보"

SELECT EMP_NAME || '의 월급은' || SALARY || '원 입니다.' AS "급여정보" FROM EMPLOYEE;

/*
DISTINCT
컬럼에 중복된 값들을 한번씩만 표기하고자 할 때 사용
*/
--employee 직급코드 조회
...
SELECT EMP_NAME, SALARY, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE = 'D1';
 ...
 SELECT EMP_NO, EMP_NAME, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE != 'D1';
 
 -- 급여가 400만원 이상인 사원들의 사원명, 부서코드, 급여 조회
 SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE SALARY >= 4000000;
 
 -- 재직중(ENT_YN 컬람값이 'N')인 사원들의 사번, 사원명, 입사일 조회
...
SELECT EMP_NAME, EMP_ID, SALARY FROM EMPLOYEE WHERE 3500000 <= SALARY AND SALARY <= 6000000;
...
/*
LIKE
비교하고자하는 컬럽값이 내가 제시한 특정 패턴에 만족 될 경우 조회
...
비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값에 문자가 "포함"되는걸 조회(키워드 검색할때 많이 씀)
...
-- 이름 중에 하 가 포함된 사원들의 사원명, 주빈번호, 전화번호 조회
...
/*
IN
비교대상컬럼값이 내가 제시한 목록 중에 일차하는 값이 있는지
...
SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE WHERE NOT DEPT_CODE IN('d6', 'd5', 'd8');
...
-- select * from employee where job_code = 'J2' or job_code = 'j7' and salary >= 2000000;
-- or 연산자보다 and 연산자가 우선이다, 괄호 쳐주면 괄호 먼저
...
-- 2. 연봉(보너스포함X)이 3000만원 이상이고 보너스를 받지 않는 사람들의 사번, 사원명, 급여, 보너스 조회
...
-- 5. 보너스 포함 연봉이 null이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스포함연봉조회(별칭)
SELECT EMP_ID, EMP_NAME, SALARY, SALARY *12 AS "보너스포함연봉조회" FROM EMPLOYEE WHERE (SALARY + (BONUS*SALARY)) * 12 IS NOT NULL AND EMP_NAME LIKE '%하%'; -- 보너스 포함 연봉!!

/*
    ORDER BY 절
    - SELECT 문 가장 마지막 줄에 작성 뿐만 아니라 실행순서 또한 마지막에 실행
    
    - 실행 순서 :  from 절 -> where절 -> select절 -> order by절
    
    [표현법]
    select 컬럼 from 테이블명 where 조건식 order by 정렬하고자 하는 컬럼 값[asc|desc][nulls first|nulls last]
    ASC : 오름차순 정렬(생략시 기본값)
    DESC : 내림차순 정렬
    
    NULLS FIRST : 정렬하고자하는컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치(생략시 DESC일때 기본값)
    NULSS LAST: 정렬하고하자흔 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치(생략 시 ASC일때 기본값)
*/

-- 전 사원의 사원명, 보너스 조회(보너스 기준 오름차순 정렬)
SELECT EMP_NAME, BONUS FROM EMPLOYEE ORDER BY BONUS DESC; -- 내림차순 정렬
--order by bonus 오름차순 정렬
--bouus desc nulls last; --보너스 기준 내림차순 정렬(null이 맨뒤)
