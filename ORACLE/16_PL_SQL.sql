...
-- 문제로 풀어보기!
-- 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL을 선언하고
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('부소 : ' || DTITLE);
...
/*
   ...
   END IF;
*/
-- 사용자에게 점수를 입력받아 SCORE 변수(NUMBER)에 저장한 후
--   학점은 입력된 점수에 따라 GRADE 변수(CHAR(1))에 저장
...
BEGIN
    ...
    INTO GRADE
    FROM SAL_GRADE
    WHERE SAL BETWEEN MIN_SAL AND MAN_SAL;
    DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 ' || GRADE || '입니다');
END;
/
...
-- DEPT_CODE에 따라서 알맞은 부서를 출력
--(부서의 변수명은 DNAME, 데이터타입은 VARCHAR2(30))
-- D1인 경우 인사관리부, D2인 경우 회계 관리부, D3인 경우 마케팅부,
-- D4인 경우 국내영업어, D5인 경우 해외영업1부, D6인 경우 해외영업2부
...
BEGIN
    ...
    LOOP 
        ...
        FOR NUM1 IN 1..9
        LOOP
        DBMS_OUTPUT.PUT_LINE(NUM || '*' || NUM1 || '=' || NUM*NUM1);
        END LOOP;
