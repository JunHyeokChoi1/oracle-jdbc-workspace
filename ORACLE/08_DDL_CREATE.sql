...
오라클에서 객체(구조): 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQEUNCE)
                    인덱스(INDEX), 패키지(PACKAGE), 트리거(TRIGGER)
                    프로시저(PROCEDURE), 함수(FUNTION), 동의어(SYNONYM),
                    사용자(USER)
*/
...
INSERT INTO MEMBER VALUES(1, 'user01', 'pass01', '홍길동', 
                        '남', '010-1111-2222', 'aaa@naver.com', '23/06/26');
...
INSERT INTO MEM_NOTNULL VALUES(2, 'user02', null, '김말순', '여', null, 'aaa_naver.com');
--> NOT NULL제약조건에 위배되어 오류 발생!
...
/*
CHECK(조건식) 제약조건
해당 컬럼에 들어올 수 있는 값데 대한 조건을 제시해볼 수 있음
...
NOT NULL + UNIQUE 제약조건이 설정가능
한 테이블당 오로지 한개만 설정 가능
*/

CREATE TABLE MEM_PRI( 
    MEM_NO NUMBER, --CONTRAINT MEMNO_PK PRIMARY KEY, --> 컬럼레벨방식
    MEM_ID VARCHAR2(20) NOT NULL,
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
...
CREATE TABLE MEM_PRI2( 
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    PRIMARY KEY(MEM_NO, MEM_ID) --묶어서 PROMARY KEY 제약조건 부여
                                --(복합키)
);
...
--> 유효한 회원등급번호가 아님에도 불구하고 INSSERT! 아직 연결 X

SELECT * FROM MEM;

/*
FOREIGN KEY(외래키) 제약조건
외래키 역학을 하는 컬럼에 부여하는 제약조건
...
--> 참조할컬럼명 생략시 참조할테이블명에 PRIMARY KEY로 지정되 컬럼으로 매칭
*/
DROP TABLE MEM;
CREATE TABLE MEM(
    MEM_NO NUMBER PRIMARY KEY,
...
-- 부모테이블로부터 무조건 삭제가 안되는 "삭제제한" 옵션이 결려있음
