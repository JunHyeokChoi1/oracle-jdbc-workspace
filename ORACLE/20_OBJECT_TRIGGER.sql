/*
    ...
    ex)
        ...
        > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할 때마다 매번 트리거 실행(FOR EACH ROW 옵션 기술해야됨)
            ...
            > :NEW - ALTER INSERT(추가된 자료), AFTER UPDATE(수정후 자료)
    ...
    DECLARE
        변수선언;
    BEGIN
        실행내용(해당 위에 지정된 이벤트 발생시 자동으로 실행한 구문);
    EXCEPTION
        예외처리구문;
    END;
    /
...
UPDATE TB_PRODUCT
SET STOCK = STOCK - 5
WHERE PCODE = 2; --> 일부러 실수

-- TB_PRODETAL 테이블에 INSERT 이벤트 발생시
